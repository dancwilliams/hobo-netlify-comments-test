...
function saveInitStatus(initStatus) {
  fs.writeFile(buildSrc + "/content/init.json", JSON.stringify(initStatus), function(err) {
    if(err) {
      console.log(err);
    }
  });
...
*/
gulp.task('generate', shell.task('hugo --gc --minify'));
...
gulp.task('build', gulp.series('get:comments', 'check-init', 'generate'))


/*
  Let's build this for local development
*/
gulp.task('build:local', gulp.series('generate'))